(() => {
  ...
  const report = () => {
    ...
    try {
      chrome.runtime.sendMessage({ type: 'facebook-marketplace-form-report', report: result }).catch(() => {});
    } catch {
  ...
  // Marketplace renders the form client-side; wait for a quiet DOM, but never observe forever.
  let quietTimer;
  let maxTimer;
  let observer;
  let finished = false;
