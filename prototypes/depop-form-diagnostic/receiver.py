#!/usr/bin/env python3
"""Small localhost receiver for the throwaway Depop diagnostic."""
...
REPORT_DIR = Path(os.environ.get("DEPOP_REPORT_DIR", "/tmp/depop-form-reports"))
...
def is_create_path(path):
    return path == "/products/create" or path.startswith("/products/create/")
...
def validate_report(report):
    ...
    if (not parsed or len(url) > 512 or parsed.scheme != "https" or
            parsed.netloc != "www.depop.com" or not is_create_path(parsed.path) or
            parsed.query or parsed.fragment):
        raise ValueError("report URL is not a clean Depop create-listing path")
